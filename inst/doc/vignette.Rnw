...
three default terms will be parsed: complex by agrep, \texttt{\\Base\\b} by grep, and 
\texttt{\\Bsome\\b} by grep. The latter terms will search for protein complexes such as 
...
all the intermediates of the hierarchy  will also be returned (the sub-
complexes as well). 
\end{itemize}
...
to ascertain protein complex co-membership, GO and MIPS both rely on a number of different
...
was used as a bait protein in the AP-MS techonology) and multi-edge (the directed graph showed 
higher instances of reciprocity) were taken to be high quality protein
complex estimates. 
