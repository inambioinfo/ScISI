...
from the protein complex it was annotated using only these evidence codes. The default uses
the codes: "902.01.01.02.01.01.02" (co-immunoprecipitation, epitope tag),
...
ascertained without the orders of each complex at hand.
...
We remark here that there is redendancy even within a single
...
In constructing our \textit{in silico} interactome, we have chosen to
dis-allow the inclusion of protein sub-complexes. The $toBeRmSubC$ entry
...
mips2mips$toBeRmSubC[1:5]
mips2go$toBeRmSubC[1:5]
@ 
...
<<mergeBGMat, eval=TRUE>>=
  dim(mipsM)
  dim(goM)
  merged = mergeBGMat(mipsM, goM, toBeRm = c(mips2go$toBeRm, mips2mips$toBeRm, go2go$toBeRm,
                                             mips2go$toBeRmSubC, mips2mips$toBeRmSubC, 
                                             go2go$toBeRmSubC))
  dim(merged)
...
linkNames[i] = Desc(mipsOb, goNames[i])
}
ScISI2html(urlVect = url, linkName = linkNames, filename = "go.html", title = "GO Protein Complex")
