...
systematic gene names, while others might 
map to other nomenclatures that need further mapping (though none of these 
maps need necessarily be one to one). Some
IntAct protein codes cannot be mapped to systematic gene names, and for these proteins,
the IntAct codes is kept.
...
unique and stable path structure, a rooted, directed tree 
Figure~\ref{eg}. The choice will be made to take the systematic gene name that is 
the destination of the left-most path in this tree, in Figure~\ref{eg}, 
\Sexpr{egEBI16112@nodes[1]} will map to \Sexpr{egEBI16112@nodes[4]}. 
...
Need to still write about these two tables...generate the Y2H interactome and
write about it...this is where I am so the following is un-edited still.
