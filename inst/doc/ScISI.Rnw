...
helping to better elucidate the nature and number of interactions
present \cite{Gavin, Ho, Krogan, Ito, Uetz}. Yet, for all organisms or
cells our knowledge is very incomplete. Hence, many more experiments
will be needed to fully reveal the network structure. However, such
experiments are expensive and time consuming to carry out. Hence,
there is a great need for simulation models that can help to identify
...
the creation of \textit{in silica interactomes}.

We begin by distinguishing between many different types of
interactomes; we begin with general comments and make many of the
...
models protein complex co-membership, in these models two proteins are
said to interact if they form a complex, to carry out some objective,
...
experiments \cite{FIXME:somecitations}. We note, that proteins that
interact in the second sense, need not interact in the first sense and
hence our separation of these two different, but related concepts.
...
describe five different roles that such a computational device can be
used in.  First, the \textit{in silico} interactome provides a tool
...
experiments. Finally, a fourth use of an \textit{in silico}
interactome is to study the effect of perturbations in the network,
such perturbations could be caused by evolution and hence could become
part of the study of evolutionary pressure on the interactome, or they
could be due to drug treatments and hence would become part of a
...
produced and it is this construction that is our focus. There are many
...
with localy produced, but not yet public, data, or to satisfy personal
...
With this definition, a person is associated to his/her grandparent, etc.
It is trivial to see that our second example implies the first, but the 
...
homo-dimer. Let $A$ be the interactome on the set $P$ where the association is
...
due to the definition of protein complex. The reverse relationship,
...
In this paper, we will build the interactome $ScISI$. The elements of $ScISI$
...
One the sub-interatomes $M$ and $G$ are merged into a comprehensive
...
constructed the \textit{in silico interactome}, $ScISI$. The 
