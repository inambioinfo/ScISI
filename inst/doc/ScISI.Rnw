...
the ScISI prediction. Since we have made use of protein complexes
described in both GO and MIPS we cannot compare our estimates to those
sources. But rather will require less direct approach.
...
This observation suggests making use of either yeast two-hybrid data
or computational predictions of interactions in order to assess
whether there is sufficient evidence in favor of the complex being
connected.
...
We propose three different summary statistics and investigate their
behavior using the ScISI and the available Y2H data.
