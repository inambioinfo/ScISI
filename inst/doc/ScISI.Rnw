...
\subfigure[Graphical rendering of ARP 2/3 labelled by systematic names]{
\label{arp23Graph}
...
chosen the systematic gene names.}
\end{figure}
...
data repositories, GO and MIPS, we used somewhat indirect methodologies. 
For the GO data source, we made use of the Bioconductor package 
\Rpackage{GO}, a metadata package containing the pertinent information. 
...
contains the evidence codes definitions. 
...
complexes. One is the lack of uniformity by which these complexes we estimated;
...
analysis \cite{Scholtens, ScholtensVidalGent} of the resulting protein complexes 
...
\cite{Scholtens, ScholtensVidalGent} have shown that the MBME 
...
\subsubsection{Analysis and Merging of the Data-Sets}
