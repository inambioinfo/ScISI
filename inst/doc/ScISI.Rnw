...
\cite{Y2H, Ito, Uetz}. A second type of interactome is one that
...
experiments \cite{Ho, Gavin, Krogan}. We note that proteins which
...
used.  First, the \textit{in silico} interactome provides a tool that
can be used to mimic wet-lab experiments; these \textit{in silico}
experiments can help to confirm the viability of the wet-lab
...
experiments. A fourth use of an \textit{in silico} interactome is to
...
\cite{Oltavi,Goverview, SubsampNetworks} that raise many important
...
that of protein complex co-membership. In this setting all members of
...
such data is as either a hypergraph \cite{Berge} or the equivalent
bipartite graph representation. Proteins are grouped into sets
...
protien complexes are being modeled.
...
exists under another set of conditions. However, such situations will
...
attributes for the conditions we are going to simulate from.
...
To each bipartite graph comes an associated incidence matrix
representation.  This incidence matrix representation, $M_B$, is a
$\{0,1\}$-matrix where proteins index the rows and protein complexes
index the columns. The entries of $M_B$ correspond to the presence or
absence of membership edges, for instance, the $(i,j)$ entry for $M_B$
is unity if protein $p_i$ is a constituent member of complex $c_j$;
the entry is zero otherwise.

Because most computational methodologies desire this incidence matrix
representation rather than the actual bipartite graph itself, the
construction of the \textit{in silico} interactome will be the
construction of a particular incidence matrix.

\subsection{ScISI}

With publicly released data available in a number of disparate
sources, there is a need for computational tools to parse through the
data-sets and collate them into one single, non-redundant, and
meaningful database resource. To this end, we have built the $R$
package \Rpackage{ScISI} for the specific purpose of parsing through
protein interaction resources and selecting protein complex
information, i.e.  the various documented protein complexes as well as
their consistent protein members.

\Rpackage{ScISI} takes a protein interaction repository and
systematically parses through that repository's database in search for
key terms. Depending on the catalog of each repository,
\Rpackage{ScISI} parses the data and searches (either by grep or
agrep) for three key terms: 1. the exact word ``complex''; 2. the
suffix ``-ase''; and 3. the suffix ``-some''. We remark that the user
can search for any term desired though the three terms we have used
are the defaults.
...
bipartite graph incidence matrix where the rows are indexed by the standard
...
produced an \textit{in silico sub-interactome} (or a sub-bipartite graph
of the bipartite interactome graph).
...
a small number of repositories which store the bipartite graph incidence graph 
...
bipartite graph incidence matrices have been constructed, \Rpackage{ScISI} compares
...
\Rpackage{ScISI} transforms this list into a bipartite graph incidence
...
protein complexes and their constituent protein members in bipartite
