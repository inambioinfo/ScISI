...
Protein interactions form a fundamental part of molecular biology and
are relevant to our understanding of complex biological processes and
...
goal of helping to elucidate the nature and number of interactions
present \citep{Gavin, Ho, Krogan, Ito, Uetz, Giot}. Yet our knowledge is
...
different network structures (FIXME: justify this?). However, such
experiments are expensive and time consuming to carry out, suggesting
...
methodology to help design experiments and to analyze the resultant
data. In this paper we outline one approach to the creation of
...
allowing investigators to alter experimental conditions (both
stochastic and systematic errors can be modified) and to measure how
the outputs change as the error rates do. Second, the ISI provides a
tool that can generate multiple data sets, under different conditions,
so that the statistical properties of different of proposed estimation
...
that will probe the interactome at a very detailed level. For example,
...
current state of biological knowledge. It can, be used out any of the
five tasks listed above, however. Further, one type of interaction can
be used to either predict or model other types of interactions, and
