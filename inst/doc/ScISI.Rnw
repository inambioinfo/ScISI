...
\item one or more words that end with the suffic \texttt{some}.
\end{enumerate}
...
(ND); Not Recorded(NR). 
%%FIXME: I have no idea what is happening here.
...
Some of the MIPS protein complexes have been uniquely identified by
...
these protein complexes. %%FIXME: then we should do the same for GO?
