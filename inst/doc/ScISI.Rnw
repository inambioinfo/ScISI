...
$\overbar{U} = C \\ U$. If $P \in U$, then we expect there to be 
...
$C$, say, consisting of $n$ proteins there are between $n-1$ and
$n(n-1)$ edges. We can conceptualize this in the following
way. Consider an urn that contains balls, each ball represents on
possible edge, or binary interaction. For $C$ there are $n(n-1)$
possible edges and hence there are that many balls in the urn. Some of
the balls represent edges that truly exist, we can imagine that these
are black and the remainder are white. 
