...
We can relate the two previous population interactomes to biologically
interactomes. Let $P$ be the set of genes within the genome of some cell
...
and underscores the necessity to keeping distinct interactimes separate. 
...
In this paper, we will build the interactome $ScISI$. The elements of $B$
is the disjoint union of two sets: the first is the set $P$ of genes
...
We have chosen to represent the protein complex membership graph as
