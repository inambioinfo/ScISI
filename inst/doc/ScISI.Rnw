...
organization. In this paper we discuss the problem of estimating
interactomes and in particular we concetrate on two different
interactomes, one representing binary interaction data and the second
representing protein complexes. We propose treating these as
...
%%FIXME:RG says, I have no idea what that sentence means - the words
%%are ok, but the grouping of them????
...
with some particular bait protein(s) and became functional prey
proteins.

%%FIXME: please remove things we cannot map to known identifiers. The
