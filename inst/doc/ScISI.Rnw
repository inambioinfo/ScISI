...
consuming to carry out. They are subject to variation and there are
many reports (FIXME: need to fill in some background on related studies 
here) of high false negative and false postive rates. 

%%FIXME: we need to justify the separate interactomes - perhaps with
%% forward references to the Arp2/3 complexes

%%FIXME: we really need a more careful background and related methods
%%section. Some of the papers that estimate FP and FN and some of the
%% papers on estimating the interactome - how is ours different, and
%% why.
