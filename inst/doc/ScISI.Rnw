...
so-called \textit{matrix} model \cite{} to
...
\caption{The difference between the true atomic structure determined by crystallography
and by Y2H experimentation. The missing edges from the Y2H graph is indeterminate since
missing edges can either be due to un-testing or due to false negative observations.}
\end{figure}
